<html>
    <head>
        <title>title</title>
        
        <link href="css/font-awesome.min.css" rel="stylesheet" type="text/css"/>
        <link href="css/property_details.css" rel="stylesheet" type="text/css"/>
        <link href="css/bootstrap.min.css" rel="stylesheet" type="text/css"/>
       
    </head>
    <body>
        <section class="section">
            <div class=" col-md-12">
                <!--Tabs Section-->
                <section class="property-details" style="background-image:url('images/image-21.jpg');">
                    <div class="auto-container">
                        <div class="row clearfix">
                            <!--Left Column-->
                            <div class="col-md-7 col-sm-12 col-xs-12 left-column">
                                <h2>Property Details</h2>
                                <!--Tabs Box-->
                                <div class="tabs-box">
                                    <!--Tab Buttons-->
                                    <div class="tab-buttons clearfix">
                                        <a href="#tab-one179" class="tab-btn active-btn">Exterior Details</a>

                                        <a href="#tab-one178" class="tab-btn">Room Dimensions</a>

                                        <a href="#tab-one181" class="tab-btn">Interior Details</a>
                                    </div>
                                    <!--Tabs Content-->
                                    <div class="tab-content">
                                        <!--Tab / Active Tab-->
                                        <div class="tab active-tab" style="display: block">
                                            <h3>Exterior Details</h3>
                                            <div class="text">
                                                Real Estate agents are Property consisting of land and the buildings on it, along with its seds naturals resources such seds as crops, minerals, or water; immovable property of this nature.
                                            </div>
                                            <div class="row clearfix">
                                                <div class="column col-md-4 col-sm-4 col-xs-12"><!--Styled List-->
                                                    <ul class="styled-list">
                                                        <li class="fa fa-share">Water Strg : 5000/ltr</li>
                                                        <li class="fa fa-share">No of Floors : 2</li>
                                                        <li class="fa fa-share">No Air Condition : 2</li>
                                                    </ul>
                                                </div>
                                                <div class="column col-md-4 col-sm-4 col-xs-12"><!--Styled List-->
                                                    <ul class="styled-list">
                                                        <li class="fa fa-share">Bathroom : 2</li>
                                                        <li class="fa fa-share">No of Stair : 4</li>
                                                        <li class="fa fa-share">No of Kitchen : 1</li>
                                                    </ul>
                                                </div>
                                                <div class="column col-md-4 col-sm-4 col-xs-12"><!--Styled List-->
                                                    <ul class="styled-list">
                                                        <li class="fa fa-share">Bedrooms : 2</li>
                                                        <li class="fa fa-share">Parking Capacity : 4</li>
                                                        <li class="fa fa-share">Swimming Pool : 1</li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                                        <!--Tab / Active Tab-->
                                        <div class="tab" >
                                            <h3>Room Dimensions</h3>
                                            <div class="text">
                                                Real Estate agents are Property consisting of land and the buildings on it, along with its seds naturals resources such seds as crops, minerals, or water; immovable property of this nature.
                                            </div>
                                            <div class="row clearfix">
                                                <div class="column col-md-4 col-sm-4 col-xs-12"><!--Styled List-->
                                                    <ul class="styled-list">
                                                        <li class="fa fa-share">Water Strg : 5000/ltr</li>
                                                        <li class="fa fa-share">No of Floors : 2</li>
                                                        <li class="fa fa-share">No Air Condition : 2</li>
                                                    </ul>
                                                </div>
                                                <div class="column col-md-4 col-sm-4 col-xs-12"><!--Styled List-->
                                                    <ul class="styled-list">
                                                        <li class="fa fa-share">Bathroom : 2</li>
                                                        <li class="fa fa-share">No of Stair : 4</li>
                                                        <li class="fa fa-share">No of Kitchen : 1</li>
                                                    </ul>
                                                </div>
                                                <div class="column col-md-4 col-sm-4 col-xs-12"><!--Styled List-->
                                                    <ul class="styled-list">
                                                        <li class="fa fa-share">Bedrooms : 2</li>
                                                        <li class="fa fa-share">Parking Capacity : 4</li>
                                                        <li class="fa fa-share">Swimming Pool : 1</li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                                        <!--Tab / Active Tab-->
                                        <div class="tab" >
                                            <h3>Interior Details</h3>
                                            <div class="text">
                                                Real Estate agents are Property consisting of land and the buildings on it, along with its seds naturals resources such seds as crops, minerals, or water; immovable property of this nature,
                                            </div>
                                            <div class="row clearfix">
                                                <div class="column col-md-4 col-sm-4 col-xs-12"><!--Styled List-->
                                                    <ul class="styled-list">
                                                        <li class="fa fa-share">Water Strg : 5000/ltr</li>
                                                        <li class="fa fa-share">No of Floors : 2</li>
                                                        <li class="fa fa-share">No Air Condition : 2</li>
                                                    </ul>
                                                </div>
                                                <div class="column col-md-4 col-sm-4 col-xs-12"><!--Styled List-->
                                                    <ul class="styled-list">
                                                        <li class="fa fa-share">Bathroom : 2</li>
                                                        <li class="fa fa-share">No of Stair : 4</li>
                                                        <li class="fa fa-share">No of Kitchen : 1</li>
                                                    </ul>
                                                </div>
                                                <div class="column col-md-4 col-sm-4 col-xs-12"><!--Styled List-->
                                                    <ul class="styled-list">
                                                        <li class="fa fa-share">Bedrooms : 2</li>
                                                        <li class="fa fa-share">Parking Capacity : 4</li>
                                                        <li class="fa fa-share">Swimming Pool : 1</li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>
            </div>
            <div class="clearfix"></div>
        </section>
    </body>
</html>
